...
object FakeRepository {
    ...
    private fun parseFakeCreators(): List<Creator> {
        val creatorType = object : TypeToken<List<Creator>>() {}.type
        return Gson().fromJson(fakeJson, creatorType)
    }

    private val fakeCreators: List<Creator> = parseFakeCreators()

    val fakeCategories: List<Category> = listOf(
        Category(id = 1, title = "Foryou", creators = fakeCreators.shuffled().take(6)),
        Category(id = 2, title = "Trending", creators = fakeCreators.shuffled().take(5)),
        Category(id = 3, title = "Newcomers", creators = fakeCreators.shuffled().take(4)),
        Category(id = 4, title = "Artists", creators = fakeCreators.shuffled().take(6))
    )
}
